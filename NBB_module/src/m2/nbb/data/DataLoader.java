...
public class DataLoader {
    ...
    // 扩展因子，避免数据过于夸张
    final private double expansionFactor = 1.;
     ...
     * @return 估计结果
     */
    public int getResult(byte[] D) {
        int num = -1;
        double max = Double.MAX_VALUE;
        for (int i = 0; i < this.count.length; i++) {
            double tmp = this.getJudgementFactor(D, i);
            if (tmp > max) {
                max = tmp;
                num = i;
            }
     ...
     * @return 返回条件概率乘积
     */
    public double getJudgementFactor(byte[] D, int h) {
        double res = (double)this.count[h] / this.data_count;
        for (int j = 0; j < D.length; j++) {
            double tmp = this.getClassCondProb_Sub(j, D[j], h);
            if (tmp != 1.) res *= tmp * expansionFactor;
        }
        return res;
    }
