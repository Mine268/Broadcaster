...
public class DataLoader {
    ...
    DataLoader(String srcLabel, String srcImage, int N) {
        try {
    ...
    public double getClassCondProb(byte[] D, int h) {
        double res = 1.;
        for (byte b : D) res *= this.getClassCondProb_Sub(b, h);
        return res;
    }
     ...
     * @return 返回条件概率分量
     */
    private double getClassCondProb_Sub(int d, int h) {
        return (double)this.data[h][d] / this.count[h];
    }
}
